...
from transformer_encoder_Alibi_Dropout import TransformerEncoder as TransformerEncoder_Alibi
from transformer_encoder_Alibi_Dropout import FeedforwardClassifier as FeedforwardClassifier_Alibi
from transformer_encoder_Alibi_Dropout import UnifiedClassifier as UnifiedClassifier_Alibi
import sys
...
def part3_1():
    ...
    # for the classification  task, you will train for a fixed number of epochs like this:
    print("Creating encoder and classifier models ...")
    encoder = TransformerEncoder_Alibi(vocab_size, n_embd, n_head, n_layer, block_size).to(device)
    classifier = FeedforwardClassifier_Alibi(n_input, n_hidden, n_output).to(device)
    unified_classifier = UnifiedClassifier_Alibi(encoder, classifier).to(device)
    print(sum(p.numel() for p in unified_classifier.parameters()), 'parameters')
    loss_function = nn.NLLLoss()
    optimizer = optim.Adam(list(encoder.parameters()) + list(classifier.parameters()), lr=learning_rate)
    for epoch in range(epochs_CLS):
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        if option == 'part1':
            ...
            print('Running part3_1')
            part3_1()
        elif option == 'part3_2':
            print('Running part3_2')
            part3_2()

    else:
        print('Running part1 and part2 by default')
        main()
