...
class Utilities:
    ...
    def sanity_check(self, sentence, block_size):
        ...
        _, attn_maps = self.model(
            input_tensor)  # Ignore the output of the model, and only get the attention maps; make sure your encoder returns the attention maps
        ...
        for j, attn_map in enumerate(attn_maps):
            ...
            plt.title(f"Attention Map Encoder {j + 1}")

            # Save the plot
            plt.savefig(f"attention_map_encoder_{j + 1}.png")

            # Show the plot
            plt.show()

    def sanity_check_for_decoder_models(self, sentence, block_size):
        # Encode the sentence using the tokenizer
        wordids = self.tokenizer.encode(sentence)
        ...
        for j, attn_map in enumerate(attn_maps):
            ...
            ax.xaxis.tick_top()
            fig.colorbar(cax, ax=ax)
            plt.title(f"Attention Map Decoder {j + 1}")

            # Save the plot
            plt.savefig(f"attention_map_decoder_{j + 1}.png")
